...
int EwInit( void )
{
  ...
  CHECK_RESULT( EwBspDisplayInit( EwScreenSize.X, EwScreenSize.Y, &DisplayInfo ));
  ...
  PRINT_RESULT( EwBspTouchInit( EwScreenSize.X, EwScreenSize.Y, &DisplayInfo ));
      ...
      CHECK_RESULT( EwMemory );
    #endif

    /* initialize heap manager */
    EwPrint( "Initialize Memory Manager...                 " );
    CHECK_RESULT( EwInitHeap( 0 ));
  ...
  /* initialize Runtime Environment and Graphics Engine */
  EwPrint( "Initialize Platform Package...               " );
  EwInitRuntimeEnvironment( 0 );
  CHECK_RESULT( EwInitGraphicsEngine( 0 ));

  /* initialize the invocation queue to process callbacks in context of GUI thread/task */
  EwPrint( "Initialize Invocation Queue...               " );
  CHECK_RESULT( EwInitInvocations( EW_INVOCATION_QUEUE_SIZE ));
  ...
  DeviceDriver_Initialize();
    ...
    CHECK_RESULT( EwTestImmediateReclaimMemory());
  #endif
...
void EwDone( void )
{
  ...
  /* deinitialize Runtime Environment and Graphics Engine */
  EwPrint( "Deinitialize Platform Package...             " );
