...
#ifndef _EffectsRectEffect_H
#define _EffectsRectEffect_H
...
/* The class Effects::RectEffect provides an animation effect intended to animate 
   a 'rect' value within a range determined by the properties @Value1 and @value2. 
   During the animation the value is interpolated and stored in the variable @Value. 
   Additionally the @Outlet property can refer to a 'rect' property which thus will 
   be 'remote controlled' by the effect. 
   ...
   direction. */
EW_DEFINE_FIELDS( EffectsRectEffect, EffectsEffect )
  EW_PROPERTY( Outlet,          XRef )
  EW_VARIABLE( Value,           XRect )
  EW_PROPERTY( Value2,          XRect )
  EW_PROPERTY( Value1,          XRect )
EW_END_OF_FIELDS( EffectsRectEffect )

/* Virtual Method Table (VMT) for the class : 'Effects::RectEffect' */
EW_DEFINE_METHODS( EffectsRectEffect, EffectsEffect )
  EW_METHOD( Animate,           void )( EffectsRectEffect _this, XFloat aProgress )
EW_END_OF_METHODS( EffectsRectEffect )
