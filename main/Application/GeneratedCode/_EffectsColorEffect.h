...
#ifndef _EffectsColorEffect_H
#define _EffectsColorEffect_H
...
/* The class Effects::ColorEffect provides an animation effect intended to animate 
   a 'color' value within a range determined by the properties @Value1 and @value2. 
   During the animation the value is interpolated and stored in the variable @Value. 
   Additionally the @Outlet property can refer to a 'color' property which thus 
   ...
   direction. */
EW_DEFINE_FIELDS( EffectsColorEffect, EffectsEffect )
  EW_VARIABLE( Value,           XColor )
  EW_PROPERTY( Value2,          XColor )
  EW_PROPERTY( Value1,          XColor )
EW_END_OF_FIELDS( EffectsColorEffect )

/* Virtual Method Table (VMT) for the class : 'Effects::ColorEffect' */
EW_DEFINE_METHODS( EffectsColorEffect, EffectsEffect )
  EW_METHOD( Animate,           void )( EffectsColorEffect _this, XFloat aProgress )
EW_END_OF_METHODS( EffectsColorEffect )
