...
{
  #error Please provide a precision timer with microsecond resolution!
}
...
*   service routines.
*
*   The function performs an addition between the value pointed by aVariable and
*   the value passed in the parameter aArg, stores the result back in aVariable
*   and returns the value aVariable held before the addition was performed.
*
* ARGUMENTS:
*   aVariable - Pointer to a variable to modify.
*   aArg      - Value to add to the variable.
*
* RETURN VALUE:
*   Returns the old value of aVariable just before the addition was performed.
*
*******************************************************************************/
...
XInt32 EwMathRandInt32( XInt32 aValue1, XInt32 aValue2 )
{
  ...
  if ( aValue2 > aValue1 )
    return aValue1 + (((int)r * ( aValue2 - aValue1 + 1 )) / ( 0xFFFF + 1 ));
  else
    return aValue2 + (((int)r * ( aValue1 - aValue2 + 1 )) / ( 0xFFFF + 1 ));
}
