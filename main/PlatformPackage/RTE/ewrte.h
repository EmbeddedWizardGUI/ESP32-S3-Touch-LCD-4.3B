...
#define EW_RTE_VERSION 0x000F0000
...
*
* DESCRIPTION:
*   The macro EW_32BIT_ADDR() returns an unsigned integer number corresponding
*   to the address value passed in the parameter aAddress.
*
*   Note: On target systems with 64-bit pointers and 32-bit long integers, the
*         returned number includes lower 32-bit of the pointer only. This is
*         expected behavior. The macro is used only when while calculating with
*         pointers the lower part of the pointer is necessary.
*
* ARGUMENTS:
*   aAddress - Address value to obtain the corresponding number.
*
* RETURN VALUE:
*   Returns a number representing the address.
*
*******************************************************************************/
#define EW_32BIT_ADDR( aAddress )                                              \
  ((unsigned long)((char*)(aAddress) - (char*)0 ))
...
*     all embedded objects. Then follo members containing ordinary pointers to
*     other objects. Then follow members where slots are stored. Then follow all
*     data members containing property references. Finally members containing
*     references to strings are found. The array thus consist of 6 entries:
*       #0 : Offset to first embedded object.
*       #1 : Offset to the first variable/array/property containing a pointer
*            to another object.
*       #2 : Offset to the first variable/array/property containing a slot.
*       #3 : Offset to the first variable/array/property containing a property
*            reference.
...
#define EW_DEFINE_CLASS( aClass, aSuperClass, aGCInfo0, aGCInfo1, aGCInfo2,    \
  ...
  {                                                                            \
    ...
    { (int)EW_32BIT_ADDR( &((aClass)0)->aGCInfo0 ),                            \
      (int)EW_32BIT_ADDR( &((aClass)0)->aGCInfo1 ),                            \
      (int)EW_32BIT_ADDR( &((aClass)0)->aGCInfo2 ),                            \
      (int)EW_32BIT_ADDR( &((aClass)0)->aGCInfo3 ),                            \
      (int)EW_32BIT_ADDR( &((aClass)0)->aGCInfo4 ),                            \
      (int)EW_32BIT_ADDR( &((aClass)0)->aGCInfo5 )},                           \
    aClass##__Init,                                                            \
    aClass##__ReInit,                                                          \
    aClass##__Done,
...
/*******************************************************************************
* TYPE:
*   XRuntimeEnvironmentInfo
*
* DESCRIPTION:
*   The structure XRuntimeEnvironmentInfo stores all essential parameters of
*   the Runtime Environment.
*
* ELEMENTS:
*   Version - The version of the Runtime Environment as 32-bit value with the
*   major number in upper 16-bit and minor number in lower 16-bit indicating
*   the recent version of the RTE library.
*
*******************************************************************************/
typedef struct
{
...
/*******************************************************************************
* FUNCTION:
*   EwGetRuntimeEnvironmentInfo
*
* DESCRIPTION:
*   The function EwGetRuntimeEnvironmentInfo() returns a data structure
*   containing most essential parameters of the Runtime Environment. For more
*   details see the declaration XRuntimeEnvironmentInfo.
*
* ARGUMENTS:
*   None
*
...
*
*******************************************************************************/
const XRuntimeEnvironmentInfo* EwGetRuntimeEnvironmentInfo
(
  void
);
...
*
* DESCRIPTION:
*   The macro EW_CHECK_THREAD_CONTEXT() encapsulates a condition to check
*   whether the actually executed code does still act within the CPU stack
*   memory area associated to the Embedded Wizard's GUI task. When the
*   application runs out the stack or the code is executed in context of a
*   foreign thread, an error 395 is reported.
*
* ARGUMENTS:
*   None
*
...
*
*******************************************************************************/
#ifndef EW_DONT_CHECK_THREAD_CONTEXT
  #define EW_CHECK_THREAD_CONTEXT                                              \
    {                                                                          \
      extern void** EwStackStartAddress;                                       \
      extern void** EwStackEndAddress;                                         \
      void*         dummy = 0;                                                 \
      void**        ptr   = &dummy;                                            \
                                                                               \
      if ( EwStackStartAddress && (( ptr < EwStackStartAddress ) ||            \
                                   ( ptr > EwStackEndAddress )))               \
      {                                                                        \
        EwError( 395 );                                                        \
        EwPanic();                                                             \
      }                                                                        \
...
/*******************************************************************************
* FUNCTION:
*   EwInitRuntimeEnvironment
*
* DESCRIPTION:
*   The function EwInitRuntimeEnvironment() initializes the Runtime Environment.
*
*   The usage of this function is optional only in case of needing to restart
*   an already running system. In such case invoke EwInitRuntimeEnvironment()
*   after invoking EwInitHeap() and before invoking EwInitInvocations() and
*   EwInitGraphicsEngine().
*
* ARGUMENTS:
*   aArgs - Optional argument to pass to the init function of the underlying
*     platform specific adaptation layer. Actually the argument is not used
*     and should be 0.
*
* RETURN VALUE:
...
/*******************************************************************************
* FUNCTION:
*   EwDoneRuntimeEnvironment
*
* DESCRIPTION:
*   The function EwDoneRuntimeEnvironment() deinitializes the Runtime
*   Environment.
*
*   The usage of this function is optional only in case of needing to restart
*   an already running system. In such case invoke EwDoneRuntimeEnvironment()
*   before invoking EwDoneHeap() and after invoking EwDoneGraphicsEngine() and
*   EwDoneInvocations().
*
* ARGUMENTS:
*   None
...
/*******************************************************************************
* FUNCTION:
*   EwSetStackAddressArea
*
* DESCRIPTION:
*   The function EwSetStackAddressArea() remembers the given values as start
*   and end addresses of the CPU stack used by Embedded Wizard application.
*   Knowing this Embedded Wizard can perform checks and report errors if the
*   application is running out the CPU stack memory or Embedded Wizard core
*   functions are invoked in context of a foreign thread (thread using other
*   stack memory area).
*
*   The knowledge about the CPU stack area is also needed by immediate Garbage
*   Collector. In such case the Garbage Collector can be started while the
*   application is executed and can evaluate stack contents in order to mark
*   objects/strings stored on it actually.
*
* ARGUMENTS:
*   aStackBaseAddress - Base address of the CPU stack used by Embedded Wizard
*     application.
*   aStackEndAddress  - The address at the opposite end of the CPU stack used
*     by Embedded Wizard application.
*
* RETURN VALUE:
*   None
*
*******************************************************************************/
void EwSetStackAddressArea
(
  void*                        aStackBaseAddress,
  void*                        aStackEndAddress
);
...
*
* DESCRIPTION:
*   The function EwSetAppRootObject() registers the given object as the so
*   called application root object. It is the first created object descending
*   from the Mosaic class Core::Root.
*
* ARGUMENTS:
*   aObject - The object to register as application root object.
*
* RETURN VALUE:
*   None
*
*******************************************************************************/
void EwSetAppRootObject
(
  XObject           aObject
);
...
*
* DESCRIPTION:
*   The function EwGetAppRootObject() returns a pointer to the application root
*   object. This is the first created object of a class descending from Mosaic
*   class Core::Root.
*
* ARGUMENTS:
*   None
*
* RETURN VALUE:
*   Returns the application root object or NULL if no root object is available
*   or the root object is already known but it has not yet finalized its Core::
*   Root initialization.
*
*******************************************************************************/
XObject EwGetAppRootObject
(
  void
);

#define EwGetAppRootObject()                                                   \
  ((CoreRoot)EwGetAppRootObject())
...
*   application via an invocation of the function EwSetStackAddressArea().
*
* ARGUMENTS:
...
*
*   The second structure XBlobStringRes has the same function as XStringRes
*   with the unique difference to be used in BLOB files only. Concrete, the
*   BLOB version avoids pointers and uses 32-bit offset values for this
...
/* The macro EW_STRINGIZE is used to convert macro arguments in string
   literals. */
#ifndef EW_STRINGIZE
...
*
* DESCRIPTION:
*   The definitions EW_BLOB_XXX are used to identify uniquely the contents of
*   a BLOB file.
*
* ELEMENTS:
*   EW_BLOB_LANG_ID  - Unique language ID used to signal that resource or
*     constant is stored in external BLOB file.
*   EW_BLOB_TYPE_XXX - Unique ids for data types the BLOB can store inside.
*
******************************************************************************/
#define EW_BLOB_LANG_ID         0x424C4F42
#define EW_BLOB_TYPE_RESOURCE   0x01
#define EW_BLOB_TYPE_INT8       0x11
#define EW_BLOB_TYPE_INT16      0x12
#define EW_BLOB_TYPE_INT32      0x13
#define EW_BLOB_TYPE_UINT8      0x14
#define EW_BLOB_TYPE_UINT16     0x15
#define EW_BLOB_TYPE_UINT32     0x16
#define EW_BLOB_TYPE_INT64      0x17
#define EW_BLOB_TYPE_UINT64     0x18
#define EW_BLOB_TYPE_BOOL       0x19
#define EW_BLOB_TYPE_ENUM       0x1A
#define EW_BLOB_TYPE_SET        0x1B
#define EW_BLOB_TYPE_FLOAT      0x1C
#define EW_BLOB_TYPE_CHAR       0x1D
#define EW_BLOB_TYPE_COLOR      0x1E
#define EW_BLOB_TYPE_POINT      0x1F
#define EW_BLOB_TYPE_RECT       0x20
#define EW_BLOB_TYPE_STRING     0x21
...
*
* DESCRIPTION:
*   The XBlobLink type definition addresses a data entry within a BLOB file.
*   Its members UnitId and MemberId identify the data entry. The entries can
*   be evaluated as resources or constants stored outside the executable.
*
* ELEMENTS:
*   LangId     - Is always EW_LANG_BLOB to signal that the origin of the data
*     is found within a BLOB file. It is not used to select the language of a
*     resource or constant. The language selection is performed when searching
*     the BLOB directory. See also EwFindBlobItem().
*   UnitId,
*   MemberId   - IDs identifying the corresponding data within the BLOB file.
*
******************************************************************************/
typedef struct
{
...
*
* DESCRIPTION:
*   The XBlobItem type definition represents a single entry within the BLOB
*   item directory.
*
* ELEMENTS:
*   Id         - Numbers corresponding to the unit, member and language names
*     of the BLOB item.
*   DataType   - Type of the BLOB data (see EW_BLOB_TYPE_XXX).
*   DataSize   - The size of the BLOB data in bytes.
*   DataOffset - Offset to the BLOB data relative to the beginning of the BLOB
*     file.
*
******************************************************************************/
typedef struct
...
*     object.
*   ReInit - Pointer to a function containing the re-initialization code of
*     the object to execute when language is changed.
*
******************************************************************************/
...
#define EW_DECLARE_AUTOOBJECT( aAutoObject, aClass )                           \
  ...
  void aAutoObject##__ReInit( aClass _this );
...
#define EW_DEFINE_AUTOOBJECT( aAutoObject, aClass )                            \
  ...
  {                                                                            \
    ...
    (XAutoObjectInitProc)aAutoObject##__ReInit                                 \
  };
...
*
* DESCRIPTION:
*   The function EwNewColor() initializes a new color with the given values and
...
*
* DESCRIPTION:
*   The function EwNewColorHSL() initializes a new color with the given values
*   and returns the initialized color. The function calculates from the provided
*   HSL values the corresponding RGB components.
*
*   EwNewColorHSL() implements the Chora instant constructor:
*   'color_hsl(aHue,aSaturation,aLuminance)' and
*   'color_hsl(aHue,aSaturation,aLuminance,aAlpha)'.
*
* ARGUMENTS:
*   aHue        - Hue value of the color.
*   aSaturation - Saturation value of the color.
*   aLuminance  - Luminance value of the color.
*   aAlpha      - Alpha component of the color.
*
* RETURN VALUE:
*   Returns the initialized color.
...
*
* DESCRIPTION:
*   The function EwGetColorHue() returns the hue value of the color. The
*   function calculates the value from the RGB color components.
*
*   EwGetColorHue() implements the read access to the Chora instant property:
*   'color.hue'.
*
* ARGUMENTS:
*   aColor - The color to query the hue value.
*
* RETURN VALUE:
*   Returns the hue value of the color. The value lies in range 0..360.
*
*******************************************************************************/
XInt32 EwGetColorHue
(
  XColor            aColor
);
...
/*******************************************************************************
* FUNCTION:
*   EwGetColorSaturation
*
* DESCRIPTION:
*   The function EwGetColorSaturation() returns the saturation value of the
*   color. The function calculates the value from the RGB color components.
*
*   EwGetColorSaturation() implements the read access to the Chora instant
*   property: 'color.saturation'.
*
* ARGUMENTS:
*   aColor - The color to query the saturation value.
*
* RETURN VALUE:
*   Returns the saturation value of the color. The value lies in range 0..100.
*
*******************************************************************************/
XInt32 EwGetColorSaturation
(
  XColor            aColor
);
...
*
* DESCRIPTION:
*   The function EwGetColorLuminance() returns the luminance value of the color.
*   The function calculates the value from the RGB color components.
*
*   EwGetColorLuminance() implements the read access to the Chora instant
*   property: 'color.luminance'.
*
* ARGUMENTS:
*   aColor - The color to query the luminance value.
*
* RETURN VALUE:
*   Returns the luminance value of the color. The value lies in range 0..100.
*
*******************************************************************************/
XInt32 EwGetColorLuminance
(
  XColor            aColor
);
...
*
* DESCRIPTION:
*   The function EwSetColorHue() recalculates the color value by replacing its
*   actual hue by the new value aHue.
*
*   EwSetColorHue() implements the write access to the Chora instant property
*   'color.hue'.
*
* ARGUMENTS:
*   aColor - The color to change its hue value.
*   aHue   - The new hue value for the color in range 0..360.
*
* RETURN VALUE:
*   Returns a new color with the changed hue value.
*
*******************************************************************************/
XColor EwSetColorHue
(
  XColor            aColor,
  XInt32            aHue
);
...
/*******************************************************************************
* FUNCTION:
*   EwSetColorSaturation
*
* DESCRIPTION:
*   The function EwSetColorSaturation() recalculates the color value by
*   replacing its actual saturation by the new value aSaturation.
*
*   EwSetColorSaturation() implements the write access to the Chora instant
*   property 'color.saturation'.
*
* ARGUMENTS:
*   aColor      - The color to change its saturation value.
*   aSaturation - The new saturation value for the color in range 0..100.
*
* RETURN VALUE:
*   Returns a new color with the changed saturation value.
*
*******************************************************************************/
XColor EwSetColorSaturation
(
  XColor            aColor,
  XInt32            aSaturation
);
...
*
* DESCRIPTION:
*   The function EwSetColorLuminance() recalculates the color value by replacing
*   its actual luminance by the new value aLuminance.
*
*   EwSetColorLuminance() implements the write access to the Chora instant
*   property 'color.luminance'.
*
* ARGUMENTS:
*   aColor     - The color to change its luminance value.
*   aLuminance - The new luminance value for the color in range 0..100.
*
* RETURN VALUE:
*   Returns a new color with the changed luminance value.
*
*******************************************************************************/
XColor EwSetColorLuminance
(
  XColor            aColor,
  XInt32            aLuminance
);
...
/*******************************************************************************
* FUNCTION:
*   EwGetExistingAutoObject
*
* DESCRIPTION:
*   The function EwGetExistingAutoObject() provides access to an auto object
*   identified by aAutoObject. The function verifies whether the object is
*   actually instantiated and returns this instance. If no instance of the
*   desired auto object is existing, the function returns NULL.
*
*   Unlike the function EwGetAutoObject(), EwGetExistingAutoObject() returns
*   already existing auto object instances only. It does not instantiate new
*   instances. It is therefore useful to test the initialization status of
*   an auto object.
*
* ARGUMENTS:
...
*
* RETURN VALUE:
*   Returns a pointer to the existiong auto object or NULL if the object is
*   not instantiated at the moment.
*
*******************************************************************************/
XObject EwGetExistingAutoObject
(
  const XVariant* aAutoObject,
  const void*     aClass
);

#define EwGetExistingAutoObject( aAutoObject, aClass )                         \
  ((aClass)EwGetExistingAutoObject( aAutoObject, EW_CLASS( aClass )))
...
void EwSetBlobData
(
  const void*       aStartAddress,
  const void*       aEndAddress
);
...
*
*******************************************************************************/
const XBlobItem* EwFindBlobItem
(
  const XBlobLink* aBlobLink
);
...
*
* DESCRIPTION:
*   The function EwStringFormat() composes a string by replacing placeholders
*   {1}..{9} found inside aFormat string by corresponding parameters aArg1..
*   aArg9.
*
*   EwStringFormat() implements the Chora instant method:
*    'string.format(aArg1,...)'
*
* ARGUMENTS:
*   aFormat - The string containing the placeholders.
*   aArg1,
*   aArg9   - The stringhs to replace the placeholders.
*
* RETURN VALUE:
*   Returns a copy of the string aFormat with all placeholders found inside
*   the string replaced by corresponding parameters provided in the function
*   invocation.
*
*******************************************************************************/
XString EwStringFormat
(
  XString           aFormat,
  XString           aArg1,
  XString           aArg2,
...
*
* DESCRIPTION:
*   XInvokeProc is a prototype for a user implemented routine to be executed in
*   context of Embedded Wizard's own thread or task. To schedule the invocation
*   of such function use EwInvoke() or EwInvokeCopy().
*
* ARGUMENTS:
*   aData - The data passed in the invocation of EwInvoke() or EwInvokeCopy().
*
* RETURN VALUE:
*   None
*
...
*
*   1. Calculate from the given address and size the corresponding pages within
*      the Flash memory.
*
*   2. Load the complete pages into an internal, static buffer. For optimization
...
*   aSize    - The size of the area intended for the access expressed in bytes.
*     If this parameter is > 0, the function should load as much as necessary
*     Flash pages to fully include the area 'aAddress .. (aAddress+aSize)'.
*     If this parameter is <= 0, the function should limit to load only the
*     Flash page containing aAddress.
*
* RETURN VALUE:
...
*   as belonging to memory not intended to be directly addressed by the
...
*     ErrorSS()  - Two ANSI C strings enclosed between two ""
*
