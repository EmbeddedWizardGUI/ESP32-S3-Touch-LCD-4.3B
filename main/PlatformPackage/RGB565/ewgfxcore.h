...
*     projection will take place.
*   EW_TASKID_FILTER_SURFACE      - Copy an area from a source surface into an
*     area within a destination surface by applying one or more filter effect
*     on the content of the source surface.
...
*     a 4-point color gradient.
*   EW_TASKID_FILL_GRADIENT       - Fill a polygon with complex color gradient.
*
*******************************************************************************/
...
#define EW_TASKID_FILTER_SURFACE       7
#define EW_TASKID_DRAW_LINE            8
#define EW_TASKID_DRAW_TEXT            9
#define EW_TASKID_FILL_POLYGON        10
#define EW_TASKID_FILL_GRADIENT       11
...
*   Locked  - Counter how often the surface is locked actually.
...
*   Blur,
*   ClipX1..
*   ClipY2   - Unique identification for the glyph. Useful to find glyphs within
*     the cache with the objective to reuse them. Font and GlyphId identify the
*     glyph within a font. Blur and ClipX1..ClipY2 determine optional parameters
*     for a blurred version of the original glyph. Blur stores the blur radius
*     in pixel. If a clipped version of the glyph is blurred, ClipX1..ClipY2
*     specify the clipped areas at the left, top, right and bottom edges of the
*     original glyph. If the blurred glyph is not clipped, ClipX1..ClipY2 are 0.
*   Width,
...
*     If the glyph hasnot yet been loaded into the cache surface, both values
*     are < 0.
*
*******************************************************************************/
...
*     this issue.
*   Busy      - != 0 if the state machine is actually processing the issue.
*   LowerHeap,
...
*
* RETURN VALUE:
*   If successful, EwCreateConstSurface() returns the new surface otherwise 
*   null is returned.
*
*******************************************************************************/
...
/*******************************************************************************
* FUNCTION:
*   EwCreateSurfaceWrapped
*
* DESCRIPTION:
*   The function EwCreateSurfaceWrapped() creates a new surface with the given 
*   size, pixel format and pixel information. Important here is the fact that
*   the function doesn't reserve any memory to store the pixel information.
*   Instead it, the surface associates the image data passed in the parameter
*   aHandle. In other words, it 'wraps' the externally created image content.
*
...
*   not allowed to delete the contents by itself. Instead, when the surface is
*   about to be deleted, the callback function aDeleteProc is called. Within the
*   function all necessary cleanup operations can be performed to free resources
*   associated to the externally created image content.
*
*   The additional tag values serve for identification purpose and simplifies
*   the reuse of previously freed surfaces -> see the EwFindSurface() function.
*
...
*   aTag1,
*   aTag2       - Unique identification for the surface.
*   aHandle     - The externally created content of the surface. This value is
...
*     system.
*   aDeleteProc - Function to be called by Graphics Engine when the surface is
...
*
* RETURN VALUE:
*   If successful, EwCreateSurfaceWrapped() returns the new surface otherwise 
*   null is returned.
*
*******************************************************************************/
XSurface* EwCreateSurfaceWrapped
(
  int                aFormat,
  int                aWidth,
  int                aHeight,
  ...
  void*              aTag2,
  void*              aHandle,
  XDeleteSurfaceProc aDeleteProc,
  void*              aDeleteArg
);
...
*  can be passed as callback in the EwSetSurfaceCacheTags() function.
*
*  This callback function will then be called when the surface is about to be 
*  discarded from the surafce cache. In this manner the associated cleanup
*  functionality can be executed.
*
* ARGUMENTS:
*   aSurface - Surface about to be discarded from the cache.
*   aTag1,
*   aTag2    - Unique identification for the surface.
*
* RETURN VALUE:
...
*
*******************************************************************************/
typedef void (*XDiscardSurfaceProc)
(
  XSurface*         aSurface,
  void*             aTag1,
  void*             aTag2
...
/*******************************************************************************
* FUNCTION:
*   EwSetSurfaceCacheTags
*
* DESCRIPTION:
*   The function EwSetSurfaceCacheTags() initializes the surface's cache tags
*   and assigns to the surface an optional OnDiscard function. When the system
*   is about to discard the surface from the cache, the function is invoked
*   automatically.
*
* ARGUMENTS:
*   aSurface     - The affected surface.
*   aTag1,
*   aTag2        - Unique identification for the surface.
*   aDiscardProc - Callback function to invoke when the surface is discarded
*     from the surface cache.
*
* RETURN VALUE:
*   None
*
*******************************************************************************/
void EwSetSurfaceCacheTags
(
  XSurface*           aSurface,
  void*               aTag1,
  void*               aTag2,
  XDiscardSurfaceProc aDiscardProc
);
...
*
* ARGUMENTS:
*   aFullClean - If != 0, all actually unused surfaces are freed. If == 0, only
*     surfaces are freed which are not intended to remain in cache.
*
* RETURN VALUE:
*   Returns != 0, if at least one surface could be freed.
*
*******************************************************************************/
int EwCleanSurfaceCache
(
  int               aFullClean
);
...
*
* DESCRIPTION:
*   The function EwCreateGlyph() creates a new glyph with given metric values.
*   The three additional font, glyph-id and blur parameter serve for
*   identification purpose and simplify the reuse of already existing glyphs.
*
*   If there is no memory available for the new glyph, the function forces the
*   processing of still pending drawing operations and flushes the glyph cache.
*   If all the adopted measures didn't help to attend the allocation, the 
...
*   aBlur,
*   aClipX1..
*   aClipY2   - Unique identification for the glyph. See XGlyph for further
*     details.
*   aOriginX,
*   aOriginY  - Upper left corner of the glyph, relative to the current printing
*     position on the baseline.
*   aWidth,
*   aHeight   - Size of the glyph in pixel (means the size of the ink-box).
*   aAdvance  - Advance in horizontal direction added to the printing position.
*
* RETURN VALUE:
...
*
* DESCRIPTION:
*   The function EwMapGlyph() verifies whether there is an area within the glyph
*   cache surface associated to the given glyph. If this is not the case, the
*   function reserves the area. If there is no space available in this surface,
...
*   the allocation, the function fails and returns 0 (zero).
*
* ARGUMENTS:
*   aGlyph - Glyph to map inside the glyph cache surface.
*   aLoad  - The variable receives a value != 0 if the glyph pixel require to be
*     loaded into the glyph cache surface.
*
* RETURN VALUE:
*   The function returns != 0 if the mapping was successful.
*
*******************************************************************************/
int EwMapGlyph
(
...
*   the given font, id and blur values. If no glyph could be found, the
*   function fails and returns null.
*
