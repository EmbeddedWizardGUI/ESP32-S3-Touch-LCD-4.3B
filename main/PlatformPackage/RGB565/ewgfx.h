...
*     3D perspective correct projection, filters, mask, blur effect and text
*     output. All operations are performed with or without the alpha-blending.
*     Alpha-blending allows fine fading effects when several graphical objects
*     are drawn one upon an other.
*
...
#define EW_GFX_VERSION 0x000F0000
...
extern int EwMaxNoOfGlyphs;         /* = EW_MAX_NO_OF_GLYPHS macro */
...
*   Memory  - Memory used temporarily to store the glyph pixel data.
*   Size    - If != 0, the value Pixel refers a temporary memory area containing
*     a copy of the pixel data. In such case Size determines the size of this
*     memory area in byte.
*
*******************************************************************************/
typedef struct
...
*
* DESCRIPTION:
*   The structure XColorStop is used to store color information for a single
*   position within a complex linear or radial gradient.
*
* ELEMENTS:
*   Offset - Position within the gradient as value between 0.0 .. 1.0.
...
*
* DESCRIPTION:
*   The structure XColorGradient describes a gradient composed of multiple color
*   values, so-called 'color stops'. The gradient can be linear or radial.
*
* ELEMENTS:
*   IsRadial     - != 0 if the structure describes a radial gradient.
*   IsLinear     - != 0 if the structure describes a linear gradient.
*   Start        - Start position for the linear gradient or the center position
*     for a radial gradient.
*   End          - End position for the linear gradient.
*   Matrix       - 2x2 component matrix describing scaling/rotation of a radial
*     gradient.
*   Radius       - Radius for the radial gradient.
*   MaxNoOfStops - Max. number of color stops in the array Stops.
*   NoOfStops    - Number of color stops in the array Stops being used.
*   Stops        - An array of color stops. The number of entries in this array
*    corresponds to the value 'MaxNoOfStops'.
*
*******************************************************************************/
typedef struct
{
...
*
* DESCRIPTION:
*   The structure XSVGObject stores the data of a scalable vector graphic.
*   This data can be used to display the graphic.
*
* ELEMENTS:
*   Commands - A data structure containing SVG elements and attributes encoded
*     as a sequence of commands and corresponding operands. In this manner the
*     elements can be evaluated quickly while drawing the SVG.
*   IsConst  - If == 0, Commands are stored in a memory area following this
*     XSVGObject. If != 0, this XSVGObject has no ownership over Commands.
*     This is usually the case, when Commands are stored in const data.
*
******************************************************************************/
typedef struct
{
...
#define EwConfigGraphicsEngine()                                               \
  ...
  {                                                                            \
    ...
    EwMaxNoOfGlyphs                           = EW_MAX_NO_OF_GLYPHS;           \
...
*
*******************************************************************************/
const XGraphicsEngineInfo* EwGetGraphicsEngineInfo
(
  void
);
...
*
*******************************************************************************/
XSVGObject* EwCreateSVGObjectFromSVGString
(
  XString           aSVGString
);
...
*
*******************************************************************************/
XSVGObject* EwLoadSVGObject
(
  const struct XSVGRes* aResource
);
...
*
*******************************************************************************/
void EwFreeSVGObject
(
  XSVGObject*       aSVGObject
);
...
*
*******************************************************************************/
XColorGradient* EwCreateGradient
(
  XInt32            aMaxNoOfStops
);
...
*
*******************************************************************************/
XInt32 EwGetGradientMaxNoOfColorStops
(
  XColorGradient*   aGradient
);
...
*
*******************************************************************************/
XInt32 EwGetGradientNoOfColorStops
(
  XColorGradient*   aGradient
);
...
*
*******************************************************************************/
XBitmap* EwGetBlurredBitmapFromFillPath
(
  XPoint            aSize,
  XPath*            aPath,
  XBool             aFlipY,
...
*
*******************************************************************************/
XBitmap* EwGetBlurredBitmapFromStrokePath
(
  XPoint            aSize,
  XPath*            aPath,
  XBool             aFlipY,
...
*
*******************************************************************************/
XInt32 EwAdjustFilterBlurRadius
(
  XInt32            aBlurRadius
);
...
(
  ...
  XPoint            aMaskPos,
  XBool             aMaskInverted,
  XBool             aMaskBeforeBlur,
  XInt32            aBlurRadius,
  XBool             aBlurClampToEdges,
  XInt32            aMode,
  XFloat            aIntensity,
  XColor            aColor,
  XInt32            aOpacity,
...
*   aBlurRadius - Determines the intensity of the blur filter effect. The
*     value is expressed in pixel as radius to blur the corresponding glyphs.
*     If the value is <= 0, no blur filter is applied. The blur radius is
*     limited to 64 pixel.
*   aColorTL,
*   aColorTR,
