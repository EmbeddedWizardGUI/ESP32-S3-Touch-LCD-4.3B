...
*
* DESCRIPTION:
*   The XFilterSurface structure defines the parameter set for a FILTER_SURFACE
*   task.
*
*   When executed, the task should copy the given source area from the source
*   surface and place it within the destination area of the destination surface.
*   Unlike regular COPY_SURFACE task, FILTER_SURFACE performs one or more filter
*   operations on the copied surface contents. The exact filter effect results
*   from the values stored in the structure. The filters may blur, mask and tint
*   the copied contents.
*   
* ELEMENTS:
*   DstX1..DstY2     - Destination area to fill within the destination surface.
*   SrcX1..SrcY2     - Area within the source surface relative to its top-left
*     corner to use as source.
*   SrcPosX, SrcPosY - Position within the source surface to map to the origin
*     (top-left corner) of the destination area (aDstX,aDstY).
*   MaskX1..MaskY2   - Area within the mask surface relative to its top-left
*     corner to use as mask.
*   MaskPosX, 
*   MaskPosY         - Position within the mask surface to map to the origin
*     (top-left corner) of the destination area (aDstX,aDstY).
*   SrcSurface       - Source surface to apply the filter operation on.
*   MaskSurface      - Optional mask surface.
*   MaskInverted     - Should the effect of the mask be inverted?
*   MaskBeforeBlur   - Controls the order of mask/blur filters.
*   BlurClampToEdges - Controls the blur effect at the edges of Dst area.
*   Mode             - Tint color mode. See EW_TINT_MODE_XXX.
*   Intensity        - Factor to mix the tinted colors with the original colors.
*   Color            - Color to be used by the tint filter.
*   Opacity          - Opacity to modulate the resulting color values.
*
*******************************************************************************/
typedef struct
{
  ...
  short             MaskPosY;
  XSurface*         SrcSurface;
  XSurface*         MaskSurface;
  char              MaskInverted;
  char              MaskBeforeBlur;
  short             BlurRadius;
  char              BlurClampToEdges;
  short             Mode;
  float             Intensity;
  unsigned int      Color;
  short             Opacity;
...
*   the associated X and Y positions are stored in the memory area at the end
*   of this parameters structure. Positions are represented by 32-bit signed
*   integers. Glyphs are refereces to the corresponding XGlyph struct as shown
*   below:
*
*   +-----------+---+---+-------+---+---+-------+---+---+-------+-----
*   | XDrawText | X | Y | Glyph | X | Y | Glyph | X | Y | Glyph |  ...
*   +-----------+---+---+-------+---+---+-------+---+---+-------+-----
*
...
*
* DESCRIPTION:
*   The XFillGradient structure defines the parameters set for a FILL_GRADIENT
*   task.
*
*   When executed, the task should fill a polygon resulting from the given path
*   information with a color gradient and store it within the destination area
*   of the destination surface.
*
*   The path information is stored in the memory at the end of this parameters
*   structure. It is an array of int values starting with the number of edges a
*   path is composed of. Then follow the coordinates of all path corners as X,Y
*   pairs. After the last coordinate pair next path can follow starting again
*   with the number of edges. The end of the path data is signed with 0. The X,Y
*   coordinates are stored as signed integer with 4-bit fixpoint precision:
*
*   +-------+------+------+------+------+------+------+-------+     +-----+
*   | edges |  X0  |  Y0  |  X1  |  Y1  |  X2  |  Y2  | edges | ... |  0  |
*   +-------+------+------+------+------+------+------+-------+     +-----+
...
*     surface, all path coordinates need to be converted just before performing
*     the operation.
*   IsRadial        - != 0 if the path should be filled with a radial gradient.
*     In such case StartX, StartY determine the center position of a circle with
*     given Radius. If this value is == 0, the path is filled with a linear
*     gradient interpolated along a straight line between position aStart and
*     aEnd.
*   StartX, StartY - Determines the start position for the gradient.
*   EndX, aEndY    - Determines the end position of a linear gradient.
*   Radius         - Determines the radius of a circle in case of a radial
*     gradient.
*   Matrix         - 2x2 matrix describing scaling/rotation of the gradient
*     coordinate system.
*   NoOfStops      - Number of color stops in the array Stops.
*   Stops          - Array with color stops. The entries in the array are sorted
*     according to the stop offset values.
*
*******************************************************************************/
typedef struct
{
